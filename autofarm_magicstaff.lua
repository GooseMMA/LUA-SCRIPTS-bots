-- ============================================================
-- ULTIMATE FARM v35.1: SYNTAX FIXED
-- Исправлена синтаксическая ошибка (опечатка ==) в фазе storage_move
-- ============================================================
...
-- ============================================================
-- 🛠️ АДАПТИРОВАННАЯ ФУНКЦИЯ ОТПРАВКИ (ИЗ СКРИПТА БАБОЧЕК)
-- ============================================================
local function sendToWebhook(text)
    ...
    log("[DISCORD] 📦 Отправка пакета данных на сервер Дискорд...")
    local ok, err = pcall(function()
        http.post(CONFIG.DISCORD_WEBHOOK, { 
            json = { content = text }, 
            timeout = 10000 
        })
    end)
    ...
    if ok then
        log("[DISCORD] ✅ Пакет успешно передан в http.post.")
    else
        log("[DISCORD] ❌ Ошибка выполнения http.post: " .. tostring(err))
    end
end
...
local function sendInitialReport()
    ...
    if #botIds == 0 then 
        log("[DISCORD] ⏳ Ожидание загрузки ботов в память софта для генерации старта...")
        return false 
    end
    
    log("[DISCORD] 📝 Сборка стартового отчёта для отправки...")
    
    local text = "```\n"
    text = text .. "=============================================================================\n"
    text = text .. "                🚀 СИСТЕМА УСПЕШНО ЗАПУЩЕНА (Ultimate v35.1)               \n"
    text = text .. "=============================================================================\n"
    text = text .. string.format(" 🤖 Всего ботов в сессии: %d | 📅 Время старта: %s\n", #botIds, os.date("%Y-%m-%d %H:%M:%S"))
    text = text .. "-----------------------------------------------------------------------------\n"
    text = text .. " ДЕТАЛЬНЫЙ СТАТУС И ИНВЕНТАРЬ БОТОВ НА СТАРТЕ:\n"
    text = text .. "-----------------------------------------------------------------------------\n"
    ...
    for _, id in ipairs(botIds) do
        ...
        if b and b:connected() then
            status = safeCall(b.state, b) or "Connected"
        end

        text = text .. string.format(" ▶️ Бот: ID [%s] | Ник: %s | Статус: %s\n", id, b_name, status)
        text = text .. "    🎒 Содержимое инвентаря:\n"
            ...
            if inv and #inv > 0 then
                ...
                for _, item in ipairs(inv) do
                    if item.amount and item.amount > 0 then
                        text = text .. string.format("      ▪️ ID: %-5d | Кол-во: %d\n", item.id, item.amount)
                        has_items = true
                    end
                end
                if not has_items then text = text .. "      (Инвентарь пуст)\n" end
            else
                text = text .. "      (Не удалось прочитать инвентарь)\n"
            end
        else
            text = text .. "      (Бот недоступен)\n"
        end
        text = text .. "-----------------------------------------------------------------------------\n"
    end
    text = text .. "=============================================================================\n```"
...
-- ============================================================
-- 📡 ОБНОВЛЁННАЯ ТАБЛИЦА СТАТИСТИКИ
-- ============================================================
local function sendDiscordSpamReport()
    if is_discord_sending then 
        log("[DISCORD] ⏳ Предыдущий отчёт ещё отправляется, пропускаем тик.")
        return 
    end
    
    is_discord_sending = true
    log("[DISCORD] 📊 Сборка текущей таблицы статистики...")

    local text = "```\n"
    text = text .. "=============================================================================\n"
    text = text .. "                🤖 ОБНОВЛЯЕМЫЙ ОТЧЁТ ФЕРМЫ (Ultimate v35.1)                \n"
    text = text .. "=============================================================================\n"
    ...
    text = text .. "-----------------------------------------------------------------------------\n"
    text = text .. " ID  | Имя Бота       | Текущий Мир (X,Y)   | Целевой ID| Миры  | Ср. Время  \n"
    text = text .. "-----------------------------------------------------------------------------\n"
    ...
    for _, id in ipairs(botIds) do
        ...
        local avg_time_str = "Calculating"
        local inv_dump = "    🎒 Полный Инвентарь: "
        ...
        if b and st then
            ...
            local total_cleared = st.worlds_done or 0
            local current_run = st.worlds_cleared_count or 0
            progress_str = string.format("%d/8 [%d]", current_run, total_cleared)

            if st.total_cleared_worlds and st.total_cleared_worlds > 0 then
                local avg_ms = st.total_clear_time_ms / st.total_cleared_worlds
                local avg_sec = math.floor(avg_ms / 1000)
                if avg_sec >= 60 then
                    avg_time_str = string.format("%dm %ds", math.floor(avg_sec / 60), avg_sec % 60)
                else
                    avg_time_str = string.format("%ds", avg_sec)
                end
            else
                avg_time_str = "Farming..."
            end
            ...
            if inv and #inv > 0 then
                ...
                for _, item in ipairs(inv) do
                    if item.amount and item.amount > 0 then
                        table.insert(items_table, string.format("[%d: %d]", item.id, item.amount))
                    end
                end
                if #items_table > 0 then
                    inv_dump = inv_dump .. table.concat(items_table, ", ")
                else
                    inv_dump = inv_dump .. "пусто"
                end
            else
                inv_dump = inv_dump .. "ошибка чтения"
            end
        else
            inv_dump = inv_dump .. "бот оффлайн"
        end

        text = text .. string.format(" %-3s | %-14s | %-11s (%-5s) | %-9s | %-5s | %-10s\n", 
            id, b_name, world_str, coords_str, items_str, progress_str, avg_time_str)
        text = text .. inv_dump .. "\n"
        text = text .. "-----------------------------------------------------------------------------\n"
    end
    text = text .. "=============================================================================\n```"
...
while true do
    ...
    -- Проверка отправки стартового отчёта
    if not initial_report_sent then
        if sendInitialReport() then
            initial_report_sent = true
            log("[DISCORD] ✅ Стартовый отчёт успешно отправлен.")
            last_discord_update = now
        end
    end

    -- Проверка отправки регулярной таблицы через 2 минуты
    if now - last_discord_update > CONFIG.TABLE_UPDATE_MS then
        log("[DISCORD] ⏰ Прошло 2 минуты. Запуск обновления таблицы статистики.")
        last_discord_update = now
